/*
  italo/eoy/2.cpp
  Same as 1.cpp but uses a standard Laplacian shader (rd2_sim.*) instead of
  bi-Laplacian — compatible with older GPUs (Mac Pro 2010 / High Sierra).
  SIM_STEPS restored to 8 and simDt raised to 0.016 for faster pattern evolution.
*/
...
static const int SIM_STEPS  = 8;   // affordable with standard Laplacian (9 samples vs 81)

// ── Auto-reset timing ─────────────────────────────────────────────────────────
static const double AUTO_RESET_INTERVAL = 8.0;

// ── Camera / audio timing ─────────────────────────────────────────────────────
static const float TRANSITION_FROM_OUTSIDE_TO_CENTER        = 28.f;
static const float REMAIN_TIME_IN_CENTER                     =  5.f;
static const float ROTATION_TIME                             = 15.f;
static const float TRANSITION_FROM_CENTER_TO_OUTSIDE         = 28.f;
static const float CAMERA_DISTANCE                           = 140.f;
static const float SLOW_ROTATION_SPEED                       = 0.08f;
static const float STAGE_TWO_DURATION                        = 20.f;
static const int   STAGE_THREE_PALETTE                       = 6;
...
struct WorldState {
  ...
  float    feed         = 0.055f;
  float    k            = 0.062f;
  float    simDt        = 0.016f;  // faster than 1.cpp (0.009) — Laplacian is stable at higher dt
...
struct MyApp : public DistributedAppWithState<WorldState> {
  ...
  Parameter    p_simDt       {"/simDt",       "", 0.016f, 0.001f, 0.02f};
  ...
  std::vector<Palette> palettes = {
    { {0.0f,  1.0f, 1.0f},  {1.0f, 0.0f, 0.8f},  {0.0f,  0.05f, 0.12f} }, // N°0
    { {0.35f, 0.0f, 1.0f},  {1.0f, 0.0f, 0.35f}, {0.12f, 0.0f,  0.25f} }, // N°1
    { {1.0f,  1.0f, 0.0f},  {0.0f, 0.5f, 1.0f},  {0.08f, 0.04f, 0.2f } }, // N°2
    { {1.0f,  0.0f, 0.5f},  {0.0f, 1.0f, 0.9f},  {0.05f, 0.0f,  0.02f} }, // N°3
    { {0.0f,  0.8f, 1.0f},  {1.0f, 0.5f, 0.0f},  {0.0f,  0.0f,  0.12f} }, // N°4
    { {1.0f,  0.0f, 0.5f},  {0.0f, 1.0f, 0.9f},  {0.05f, 0.0f,  0.02f} }, // N°5
    { {1.0f,  1.0f, 1.0f},  {0.15f,0.15f,0.15f}, {0.0f,  0.0f,  0.0f } }, // N°6 B&W
  };
  ...
  void onCreate() override {
    ...
    // Uses rd2_sim shaders (standard Laplacian) for the simulation pass.
    // Reuses rd_display shaders (sphere tessellation + coloring) unchanged.
    simShader.compile(slurp3(dir + "/rd2_sim.vert.glsl"),
                      slurp3(dir + "/rd2_sim.frag.glsl"));
